...
def part1(lines: List[str]):
    ...
    of numbers you have. You organize the information into a table
    (your puzzle input).
    ...
    for line in lines:
        ...
        intersect = win_nums & draw_nums
        score = 2 ** (len(intersect) - 1) if intersect else 0
        # print(score)
        total += score
    return total
...
def part2(lines: List[str]):
    ...
    for i, line in enumerate(lines):
        ...
        intersect = win_nums & draw_nums
        for j in range(i + 1, i + len(intersect) + 1):
            cards[j] += cards[i]
    return sum(cards)


if __name__ == "__main__":
    with open("../inputs/04.txt", "r", encoding="utf-8") as f:
